...
#         if not all([image_id, user_question]):
#             return Response(
#                 {"error": "Missing image_id or question."}, status=status.HTTP_400_BAD_REQUEST
#             )

#         # Retrieve detection data
#         image_result = ImageUpload.objects.get(pk=image_id)
#         if not image_result:
#             return Response({"error": "Image result not found"}, status=status.HTTP_404_NOT_FOUND)
...
#         print(f"detection data from views: {detections}")
#         # query Gemini
#         try:
